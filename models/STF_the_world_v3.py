...
class STF_the_world_v3(STF):
    def __init__(self, cfg):
        ...
        self.hist_emb = nn.Sequential(
            LinearEmbedding(traj_dims, d_model),
            c(position),
        )
        self.hist_enc = Encoder(EncoderLayer(d_model, c(attn), c(ff), dropout), N)
        self.hist_dec = Decoder(DecoderLayer(d_model, c(attn), c(attn), c(ff), dropout), N)
    ...
    def forward(self, data: dict):
        ...
        hist_enc = self.hist_enc(self.hist_emb(hist),hist_mask)
        self.query_batches = self.query_embed.weight.view(1, 1, *self.query_embed.weight.shape).repeat(*hist.shape[:2],
                                                                                                                 1, 1)
        hist_out = self.hist_dec(self.query_batches,hist_enc,hist_mask,None)
        ...
        traj_mem = lane_mem[:,max_adj_lane:,:]
        lane_mem = lane_mem[:,:max_adj_lane,:]
        ...
        social_out = self.social_dec(lane_out,traj_mem,perception_mask,None)
