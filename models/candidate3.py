...
class candidate3(STF):
    def __init__(self, cfg):
        super(candidate3, self).__init__(cfg)
        ...
        self.hist_emb = LinearEmbedding(64, 64)
        self.lane_enc = Encoder(EncoderLayer(d_model, c(attn), c(ff), dropout), 2*N)
        self.lane_dec = Decoder(DecoderLayer(d_model, c(attn), c(attn), c(ff), dropout), 2*N)
        self.prediction_head = deeperChoiceHead(d_model * 2, dec_out_size, dropout)

        self.traj_net = LaneNet(
            7,
            cfg['subgraph_width_unit'],
            cfg['num_subgraph_layers'])
        ...
        self.social_fuse = nn.Sequential(
            nn.Linear(d_model+64, d_model, bias=True),
            nn.LayerNorm(d_model),
            nn.ReLU(),
            nn.Linear(d_model, d_model, bias=True))
    ...
    def forward(self, data: dict):
        ...
        origin_hist = torch.cat([hist, one_hot_hist], -1)
        ...
        self.query_batches = self.query_embed.weight.view(1, 1, *self.query_embed.weight.shape).repeat(*hist.shape[:2],
                                                                                                       1, 1)
        origin_hist = origin_hist*data['hist_mask'][:, :max_agent].unsqueeze(-1)
        hist_enc = self.traj_net(origin_hist)
        hist_emb = self.hist_emb(hist_enc)
        ...
        social_emb = torch.cat([hist_emb,social_emb],-1)
        social_emb = self.social_fuse(social_emb)
